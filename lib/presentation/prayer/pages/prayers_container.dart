...
class _PrayersContainerState extends State<PrayersContainer> {
  ...
  Future<void> savePrayerTimesToWidget({
    ...
    await HomeWidget.updateWidget(name: 'PrayerTimesWidget', iOSName: 'PrayerTimeWidget');
  }
    ...
    return Consumer3<NotificationsState, PrayerState, TimeState>(
      builder: (context, notification, prayer, time, _) {
        ...
        if (notification.isFajrNotification) {
          _notificationService.prayerNotification(
              id: AppStringConstraints.fajrNotificationID,
              title: appLocale.prayerTime,
              body: appLocale.fajr,
              dateTime: prayer.prayerTimes.fajr);
        } else {
        ...
        if (notification.isSunriseNotification) {
          _notificationService.dailyNotification(
              id: AppStringConstraints.sunriseNotificationID,
              title: appLocale.sunrise,
              body: appLocale.adDuhaTime,
              dateTime: prayer.prayerTimes.sunrise.add(const Duration(minutes: 45)));
        } else {
        ...
        if (notification.isDhuhrNotification) {
          _notificationService.prayerNotification(
              id: AppStringConstraints.dhuhrNotificationID,
              title: appLocale.prayerTime,
              body: appLocale.dhuhr,
              dateTime: prayer.prayerTimes.dhuhr);
        } else {
          _notificationService.cancelNotificationWithId(AppStringConstraints.dhuhrNotificationID);
        }
        if (notification.isAsrNotification) {
          _notificationService.prayerNotification(
              id: AppStringConstraints.asrNotificationID,
              title: appLocale.prayerTime,
              body: appLocale.asr,
              dateTime: prayer.prayerTimes.asr);
        } else {
        ...
        if (notification.isMaghribNotification) {
          _notificationService.prayerNotification(
              id: AppStringConstraints.maghribNotificationID,
              title: appLocale.prayerTime,
              body: appLocale.maghrib,
              dateTime: prayer.prayerTimes.maghrib);
        } else {
        ...
        if (notification.isIshaNotification) {
          _notificationService.prayerNotification(
              id: AppStringConstraints.ishaNotificationID,
              title: appLocale.prayerTime,
              body: appLocale.isha,
              dateTime: prayer.prayerTimes.isha);
        } else {
        ...
        if (notification.isMorningAdhkarsNotification) {
          _notificationService.dailyNotification(
              id: AppStringConstraints.morningSupplicationsNotificationID,
              title: appLocale.remind,
              body: '${appLocale.morningAdhkarsTime} 🤲',
              dateTime: prayer.prayerTimes.fajr.add(const Duration(minutes: 30)));
        } else {
          _notificationService
              .cancelNotificationWithId(AppStringConstraints.morningSupplicationsNotificationID);
        }
        if (notification.isEveningAdhkarsNotification) {
          _notificationService.dailyNotification(
              id: AppStringConstraints.eveningSupplicationsNotificationID,
              title: appLocale.remind,
              body: '${appLocale.eveningAdhkarsTime} 🤲',
              dateTime: prayer.prayerTimes.asr.add(const Duration(minutes: 30)));
        } else {
          _notificationService
              .cancelNotificationWithId(AppStringConstraints.eveningSupplicationsNotificationID);
        }
        if (notification.isNightAdhkarsNotification) {
          _notificationService.dailyNotification(
              id: AppStringConstraints.nightSupplicationsNotificationID,
              title: appLocale.remind,
              body: '${appLocale.nightAdhkarsTime} 🤲',
              dateTime: prayer.prayerTimes.isha.add(const Duration(minutes: 30)));
        } else {
          _notificationService
              .cancelNotificationWithId(AppStringConstraints.nightSupplicationsNotificationID);
        }
        if (notification.isFastThursdayNotification && time.isFasting()) {
          _notificationService.weeklyNotification(
              id: AppStringConstraints.fastThursdayNotificationID,
              title: appLocale.remind,
              body: appLocale.nearFastingDay,
              dateTime: prayer.weeklyWednesday);
        } else {
          _notificationService
              .cancelNotificationWithId(AppStringConstraints.fastThursdayNotificationID);
        }
        if (notification.isFastMondayNotification && time.isFasting()) {
          _notificationService.weeklyNotification(
              id: AppStringConstraints.fastMondayNotificationID,
              title: appLocale.remind,
              body: appLocale.nearFastingDay,
              dateTime: prayer.weeklySunday);
        } else {
          _notificationService
              .cancelNotificationWithId(AppStringConstraints.fastMondayNotificationID);
        }
        if (notification.isFridayNotification) {
          _notificationService.weeklyNotification(
              id: AppStringConstraints.fridayNotificationID,
              title: appLocale.remind,
              body: appLocale.tomorrowFriday,
              dateTime: prayer.weeklyThursday);
        } else {
        ...
        if (notification.isLastHourFridayNotification) {
          _notificationService.weeklyNotification(
              id: AppStringConstraints.lastHourFridayNotificationID,
              title: appLocale.remind,
              body: '${appLocale.lastFridayHour} 🤲🤲🤲',
              dateTime: prayer.lastFridayHour);
        } else {
          _notificationService
              .cancelNotificationWithId(AppStringConstraints.lastHourFridayNotificationID);
        }
        if (notification.isWhiteDaysNotification && time.isFasting()) {
          _notificationService.monthlyNotification(
              id: AppStringConstraints.fastWhiteDaysNotificationID,
              title: appLocale.remind,
              body: appLocale.nearWhiteDays,
              dateTime: time.twelfthyDay);
        } else {
          _notificationService
              .cancelNotificationWithId(AppStringConstraints.fastWhiteDaysNotificationID);
        }
        return Column(
